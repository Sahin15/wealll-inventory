import React from 'react';
import { Navigate, Outlet, useLocation, Link } from 'react-router-dom';
import { useAuth } from '../context/AuthContext';
import { useGlobalSettings } from '../context/GlobalSettingsContext';
import { Building2, LogOut, FileText, Settings } from 'lucide-react';

const SuperAdminLayout = () => {
  const { user, logout, loading } = useAuth();
  const { globalSettings, loadingSettings } = useGlobalSettings();
  const location = useLocation();

  if (loading || loadingSettings) {
    return <div className="flex h-screen items-center justify-center bg-gray-50">Loading...</div>;
  }

  if (!user) {
    return <Navigate to="/login" replace />;
  }

  if (user.role !== 'superadmin') {
    return <Navigate to="/" replace />;
  }

  return (
    <div className="min-h-screen bg-gray-900 flex">
      {globalSettings?.announcementText && (
        <div className="fixed top-0 left-0 right-0 z-50 bg-indigo-600 px-4 py-3 text-white sm:px-6 lg:px-8 shadow-md">
          <p className="text-center text-sm font-medium">
            {globalSettings.announcementText}
          </p>
        </div>
      )}

      {/* Sidebar */}
      <div className={`hidden md:flex md:w-64 md:flex-col md:fixed md:inset-y-0 bg-gray-900 border-r border-gray-800 ${globalSettings?.announcementText ? 'mt-12' : ''}`}>
        <div className="flex-1 flex flex-col min-h-0">
          <div className="flex items-center h-16 flex-shrink-0 px-4 border-b border-gray-800">
            <h1 className="text-xl font-bold tracking-tight text-white">WeAlll Admin</h1>
          </div>
          <div className="flex-1 flex flex-col overflow-y-auto">
            <nav className="flex-1 px-2 py-4 space-y-1">
              <Link 
                to="/wealll-admin"
                className={`group flex items-center px-2 py-2 text-sm font-medium rounded-md ${location.pathname === '/wealll-admin' ? 'bg-gray-800 text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white'}`}
              >
                <Building2 className="mr-3 flex-shrink-0 h-5 w-5" aria-hidden="true" />
                Tenants
              </Link>
              <Link 
                to="/wealll-admin/applications"
                className={`group flex items-center px-2 py-2 text-sm font-medium rounded-md ${location.pathname === '/wealll-admin/applications' ? 'bg-gray-800 text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white'}`}
              >
                <FileText className="mr-3 flex-shrink-0 h-5 w-5" aria-hidden="true" />
                Applications
              </Link>
              <Link 
                to="/wealll-admin/plans"
                className={`group flex items-center px-2 py-2 text-sm font-medium rounded-md ${location.pathname === '/wealll-admin/plans' ? 'bg-gray-800 text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white'}`}
              >
                <FileText className="mr-3 flex-shrink-0 h-5 w-5" aria-hidden="true" />
                Plans & Subscriptions
              </Link>
              <Link 
                to="/wealll-admin/settings"
                className={`group flex items-center px-2 py-2 text-sm font-medium rounded-md ${location.pathname === '/wealll-admin/settings' ? 'bg-gray-800 text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white'}`}
              >
                <Settings className="mr-3 flex-shrink-0 h-5 w-5" aria-hidden="true" />
                Global Settings
              </Link>
            </nav>
          </div>
          <div className="flex-shrink-0 flex border-t border-gray-800 p-4">
            <div className="flex items-center w-full">
              <div className="ml-3 flex-1">
                <p className="text-sm font-medium text-white">{user.name}</p>
                <p className="text-xs font-medium text-gray-400 truncate">{user.email}</p>
              </div>
              <button onClick={logout} className="ml-auto flex-shrink-0 p-1 text-gray-400 hover:text-white rounded-full">
                <LogOut className="h-5 w-5" />
              </button>
            </div>
          </div>
        </div>
      </div>

      {/* Main Content */}
      <div className={`md:pl-64 flex flex-col flex-1 min-w-0 bg-gray-50 ${globalSettings?.announcementText ? 'pt-12' : ''}`}>
        <main className="flex-1 overflow-y-auto">
          <div className="py-6 px-4 sm:px-6 md:px-8">
            <Outlet />
          </div>
        </main>
      </div>
    </div>
  );
};

export default SuperAdminLayout;
