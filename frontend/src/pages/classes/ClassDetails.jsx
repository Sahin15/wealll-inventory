...
const ClassDetails = () => {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
                    ...
                    <div className="flex items-center justify-between pt-1">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <button
                          type="button"
                          onClick={() => toggleAttendance(student._id, student.attended)}
                          className={`inline-flex items-center justify-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-bold transition-all min-h-[40px] cursor-pointer active:scale-95 ${
                            student.attended
      ...
      {/* ========================================================================= */}
      {/* ENROLL NEW STUDENT MODAL (AdaptiveSheet)                                   */}
      {/* ========================================================================= */}
      <AdaptiveSheet
        isOpen={isEnrollModalOpen}
        onClose={closeEnrollModal}
        title="Enroll New Student"
        subtitle={`Register attendee for ${batch?.batchNumber} • Seat Fee: ${formatCurrency(batch?.seatPrice || 0)}`}
        maxWidth="max-w-lg"
        footer={
          <div className="flex items-center justify-end gap-2.5 w-full">
            <button
              ...
              className="px-4 py-2.5 border border-gray-200 rounded-xl text-xs sm:text-sm font-semibold text-gray-700 hover:bg-gray-50 transition-all cursor-pointer min-h-[44px]"
            >
              Cancel
            </button>
            <button
              ...
              disabled={submittingStudent}
              className="inline-flex items-center justify-center px-5 py-2.5 bg-rose-600 hover:bg-rose-700 text-white text-xs sm:text-sm font-bold rounded-xl shadow-md shadow-rose-200 transition-all cursor-pointer disabled:opacity-50 min-h-[44px]"
            >
              {submittingStudent ? (
                <span>Enrolling...</span>
              ) : (
      ...
      >
        <form id="enroll-student-form" onSubmit={handleAddStudent} className="p-4 sm:p-6 space-y-4">
          {/* Full Name */}
          <div>
            ...
            <input
              ...
              className="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500 focus:bg-white transition-all font-medium text-gray-900 min-h-[44px]"
            />
          </div>

          {/* Phone & Initial Payment Status */}
            ...
            <div className="bg-rose-50/50 p-4 rounded-2xl border border-rose-100 space-y-3">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 bg-white border border-gray-200 rounded-xl text-xs font-medium min-h-[44px]"
                  >
                ...
                {studentForm.paymentStatus === 'Partial' && (
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 bg-white border border-rose-300 rounded-xl text-xs font-bold text-rose-700 focus:ring-2 focus:ring-rose-500 focus:outline-none min-h-[44px]"
                    />
                  </div>
                )}
              </div>
              ...
              {studentForm.paymentStatus === 'Partial' && (
                <div className="flex justify-between items-center text-xs text-gray-600 pt-2 border-t border-rose-200/60">
                  <span>Tuition: <strong className="text-gray-900">{formatCurrency(batch?.seatPrice || 0)}</strong></span>
                  <span>Advance: <strong className="text-emerald-700">{formatCurrency(Number(studentForm.paidAmount) || 0)}</strong></span>
                  <span>Balance Due: <strong className="text-rose-600">{formatCurrency(Math.max(0, (batch?.seatPrice || 0) - (Number(studentForm.paidAmount) || 0)))}</strong></span>
                </div>
              )}
            </div>
          )}
        ...
        footer={
          <div className="flex items-center justify-end gap-2.5 w-full">
            <button
              ...
              className="px-4 py-2.5 text-xs sm:text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-xl hover:bg-gray-50 transition cursor-pointer min-h-[44px]"
            >
              Cancel
            </button>
            <button
              type="submit"
              form="details-tuition-form"
      ...
      >
        {collectPaymentStudent && (
          <form id="details-tuition-form" onSubmit={handleRecordTuitionPayment} className="p-4 sm:p-6 space-y-4">
      ...
      <AdaptiveSheet
        ...
        subtitle={selectedStudent ? `Phone: ${selectedStudent.phone || 'N/A'} • Student Purchase History` : ''}
        maxWidth="max-w-2xl"
        footer={
          selectedStudent ? (
            <div className="flex items-center justify-between w-full gap-2">
              <Link 
                ...
                className="inline-flex items-center px-4 py-2.5 bg-rose-600 hover:bg-rose-700 text-white text-xs sm:text-sm font-bold rounded-xl shadow-xs transition-all min-h-[44px]"
              >
          ...
          <div className="p-4 sm:p-6 space-y-4">
            {loadingSales ? (
              ...
              <div className="space-y-4">
                ...
                <div className="bg-gradient-to-r from-rose-500 to-pink-600 text-white rounded-2xl p-4 sm:p-5 shadow-sm flex items-center justify-between">
                  <div>
